...
class FastGroupsService {
  ...
  removeFastGroupNode(node) {
    ...
    if (index > -1) this.fastGroupNodes.splice(index, 1);
  ...
  getBoundingsForAllNodes() {
    if (!this.cachedNodeBoundings) {
      ...
      while (stack.length > 0) {
        ...
        let bounds = node.getBounding?.() ?? [0, 0, 0, 0];
        if (bounds[0] === 0 && bounds[1] === 0 && bounds[2] === 0 && bounds[3] === 0) {
          const ctx = node.graph?.primaryCanvas?.canvas?.getContext?.("2d");
          if (ctx) {
            node.updateArea?.(ctx);
            bounds = node.getBounding?.() ?? bounds;
          }
        }
  ...
  recomputeInsideNodesForGroup(group) {
    ...
    for (const node of nodes) {
      ...
      const nodeCenter =
        nodeBounding && [nodeBounding[0] + nodeBounding[2] * 0.5, nodeBounding[1] + nodeBounding[3] * 0.5];
    ...
    if (skipOtherNodeCheck !== true) {
      if (newValue && this.node.properties?.[PROPERTY_RESTRICTION]?.includes(" one")) {
        for (const widget of this.node.widgets) {
          if (widget instanceof FastGroupsToggleRowWidget) widget.doModeChange(false, true);
        }
...
class FastGroupsMuterWithAutoreset extends LGraphNode {
  ...
  onRemoved() {
    ...
    api.removeEventListener?.("execution_success", this._onExecutionComplete);
    api.removeEventListener?.("executed", this._onExecutionComplete);
  }

  _onExecutionComplete(_event) {
    try {
  ...
  refreshWidgets() {
    ...
    for (const group of groups) {
      if (filterColors.length) {
        ...
        if (groupColor.length === 3) groupColor = groupColor.replace(/(.)(.)(.)/, "$1$1$2$2$3$3");
      ...
      <ul>
        <li><p><strong>Properties.</strong></p>
          <ul>
            ...
            <li><p><code>${PROPERTY_SHOW_NAV}</code> - Show nav arrow. <i>(default: true)</i></p></li>
            <li><p><code>${PROPERTY_SHOW_ALL_GRAPHS}</code> - Show groups from all [sub]graphs. <i>(default: true)</i></p></li>
